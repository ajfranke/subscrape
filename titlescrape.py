...
while back <= finish:  
    ...
    for result in results:
        count += 1
        wordlist.append( result.title )
        postlist.append( result.id )
        timelist.append( result.created_utc )
        scorelist.append( result.score )
...
bigframe.to_csv("Homebrewing_titles_%d_%d.csv"%(start,finish),
               sep='\t',
               encoding='utf-8')
